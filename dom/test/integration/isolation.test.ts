...
describe('isolateSource', function() {
  it('should have the same effect as DOM.select()', function(done) {
    ...
    // Make assertions
    isolatedDOMSource.select('.bar').elements().skip(1).take(1).observe((elements: Array<HTMLElement>) => {
...
describe('isolateSink', function() {
  it('should add an isolate field to the vtree sink', function(done) {
    ...
    sinks.DOM.take(1).observe(function(vtree: VNode) {
      ...
      assert.strictEqual(vtree.scope, '$$MOTORCYCLEDOM$$-foo')
      setTimeout(() => {
        dispose()
        done()
      })
  ...
  it('should not redundantly repeat the scope className', function(done) {
    ...
    sinks.DOM.skip(2).take(1).observe(function(vtree: VNode) {
      ...
      assert.strictEqual(vtree.scope, '$$MOTORCYCLEDOM$$-1')
      dispose()
      done()
    })
...
describe('isolation', function() {
  it('should prevent parent from DOM.selecting() inside the isolation', function(done) {
    ...
    sources.DOM.select('.bar').elements().skip(1).take(1).observe(function(elements: Array<HTMLElement>) {
  ...
  it('should allow parent to DOM.select() in its own isolation island', function(done) {
    ...
    sinks.island.skip(1).take(1).observe(function(elements: Array<HTMLElement>) {
      assert.strictEqual(Array.isArray(elements), true)
      assert.strictEqual(elements.length, 1)
      const correctElement = elements[0]
      assert.notStrictEqual(correctElement, null)
  ...
  it('should allow a child component to DOM.select() its own root', function(done) {
    ...
    isolateSource(sources.DOM, 'ISOLATION')
      .select('.foo').elements()
      .skip(1).take(1)
      .observe(function(elements: Array<HTMLElement>) {
        assert.strictEqual(Array.isArray(elements), true)
        assert.strictEqual(elements.length, 1)
        const correctElement = elements[0]
        assert.notStrictEqual(correctElement, null)
  ...
  it('should allow DOM.selecting svg elements', function(done) {
    ...
    // Make assertions
    sinks.triangleElement.skip(1).take(1).observe((elements: Array<HTMLElement>) => {
      assert.strictEqual(elements.length, 1)
  ...
  it('should select() isolated element with tag + class', function(done) {
    ...
    // Make assertions
    isolatedDOMSource.select('h4.bar').elements().skip(1).take(1).observe((elements: Array<HTMLElement>) => {
      assert.strictEqual(elements.length, 1)
      const correctElement = elements[0]
      assert.notStrictEqual(correctElement, null)
      assert.notStrictEqual(typeof correctElement, 'undefined')
  ...
  it('should stop bubbling the event if the currentTarget was removed', function(done) {
    ...
    function main(sources: AppSources) {
      ...
      return {
        DOM: childExistence$.map((exists: boolean) =>
          div([
            div('.top-most', { scope: '$$MOTORCYCLEDOM$$-top' }, [
              h2('.bar', 'Wrong'),
              exists ? div({ scope: '$$MOTORCYCLEDOM$$-foo' }, [
                h4('.bar', 'Correct'),
              ]) : null,
            ]),
          ]),
    ...
    // Make assertions
    fooDOMSource.select('.bar').elements().skip(1).take(1).observe((elements: Array<HTMLElement>) => {
      assert.strictEqual(elements.length, 1)
      const correctElement = elements[0]
      assert.notStrictEqual(correctElement, null)
      assert.notStrictEqual(typeof correctElement, 'undefined')
  ...
  it('should handle a higher-order graph when events() are subscribed', (done) => {
    ...
    function main(sources: AppSources) {
      ...
      const vnode$ = interval(50).take(1).startWith(-1)
        ...
        .map((children: Array<{ DOM: most.Stream<VNode> }>) =>
          most.combineArray<any, any, any>(
            (...childVNodes: Array<any>) => div('.parent', childVNodes),
    ...
    sources.DOM.select(':root').elements().skip(2).take(1).observe(function([root]: Array<HTMLElement>) {
  ...
  it('should handle events when child is removed and re-added', (done) => {
    ...
    sources.DOM.select(':root').elements().skip(1).observe(function([root]: Array<HTMLElement>) {
      setTimeout(() => {
  ...
  it('should allow an isolated child to receive events when it is used as ' +
    'the vTree of an isolated parent component', (done) => {
      ...
      sources.DOM.elements().skip(1).take(1).observe(([root]: Array<HTMLElement>) => {
